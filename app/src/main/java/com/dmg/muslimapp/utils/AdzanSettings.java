 ...
 * there is a possibility of data loss if a background_regis_login thread has modified
 * preferences at the same time.
 * 
 * Usage:
 * 
 * int sampleInt = AdzanSettings.getInstance(context).getInt(Key.SAMPLE_INT);
 * AdzanSettings.getInstance(context).set(Key.SAMPLE_INT, sampleInt);
 * 
 * If AdzanSettings.getInstance(Context) has been called once, you can
 * simple use AdzanSettings.getInstance() to save some precious line space.
 */
public class AdzanSettings {
  public static final PrayTime sDefaults = new PrayTime();

  private static final String SETTINGS_NAME = "adzan_settings";
  private static AdzanSettings sSharedPrefs;
  ...
  public static class Key {
     ...
     * ints, floats, doubles, longs:
	   ...
	   */
    //ALARM RELATED
    public static final String IS_ALARM_SET = "is_alarm_set_for_%s";
    ...
    public static final String IS_SOUND_SET = "ringtone_selected";
    public static final String IS_SOUND_NAME_SET = "ringtone_selected_name";
  ...
  private AdzanSettings(Context context) {
    mPref = context.getSharedPreferences(SETTINGS_NAME, Context.MODE_PRIVATE);
    mContextRef = new WeakReference<Context>(context);
  }


  public static AdzanSettings getInstance(Context context) {
    if (sSharedPrefs == null) {
      sSharedPrefs = new AdzanSettings(context.getApplicationContext());
    }
    return sSharedPrefs;
  }

  public static AdzanSettings getInstance() {
    if (sSharedPrefs != null) {
      return sSharedPrefs;
    }
  ...
  public int getCalcMethodSetFor(int index) {
    return getInt(getKeyFor(Key.CALC_METHOD, index), PrayTime.Companion.getCUSTOM());
  }
  ...
  public int getAsrMethodSetFor(int index) {
    return getInt(String.format(Key.ASR_METHOD, index), PrayTime.Companion.getSHAFII());
  }
  ...
  public int getHighLatitudeAdjustmentFor(int index) {
    return getInt(getKeyFor(Key.ADJUST_METHOD, index), PrayTime.Companion.getANGLE_BASED());
  }
  ...
  public int getTimeFormatFor(int index) {
    return getInt(getKeyFor(Key.TIME_FORMAT, index), DateFormat.is24HourFormat(mContextRef.get())? PrayTime.Companion.getTIME_24() : PrayTime.Companion.getTIME_12());
  }
